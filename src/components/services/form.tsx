...
import { Good, ServiceCalculationType, ServiceType } from "@prisma/client";
...
export default function ServiceForm({ type, customerId, goods }: Props) {
	const closeDialogRef = useRef<HTMLButtonElement>(null);

	const formSchema = CreateService;
	...
	const { execute, fieldErrors } = useAction(createService, {
		onSuccess: () => {
			toast({
									...
									<FormControl>
										...
										>
											{form.watch(`goods.${index}.goodId`)
												? goods.find(
														(good) =>
															good.id === form.watch(`goods.${index}.goodId`)
												  )?.name
												: "Select Good"}
											...
											<CommandGroup>
												{goods.map((good) => (
													<CommandItem
														...
														key={good.id}
														onSelect={() => {
															form.setValue(`goods.${index}.goodId`, good.id, {
																shouldValidate: true,
																shouldDirty: true,
															});
														}}
